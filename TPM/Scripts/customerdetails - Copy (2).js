document.addEventListener('DOMContentLoaded', () => {
    ...
    window.editEquipment = function (equipmentId) {
        ...
        if (equipment) {
            ...
            $('#equipType').val(equipment.EquipmentType || '');
    ...
    function equipmentSave(event) {
        ...
        var equipType = $('#equipType').val().trim();
        ...
        if (!equipType) {
            Swal.fire('Validation Error', 'Type is required.', 'warning');
            $('#equipType').focus();
            return;
        }
        ...
        notes.forEach(note => {
            ...
            html += `
                <tr ${note.AppointmentId ? `data-appointment-id="${note.AppointmentId}"` : ''}>
                    ...
                    <td>
                        <div class="d-flex gap-2">                        
                            <button class="btn btn-sm btn-outline-primary edit-note-btn" data-note-id="${note.Id}" title="Edit"><i class="fas fa-edit"></i></button>
                              <button class="btn btn-sm btn-outline-secondary email-note-btn" data-note-id="${note.Id}" title="Email"><i class="fas fa-envelope"></i></button>
                            <button class="btn btn-sm btn-outline-danger delete-note-btn" data-note-id="${note.Id}" title="Delete"><i class="fas fa-trash-alt"></i></button>
                        </div>
                    </td>
                </tr>
            `;
